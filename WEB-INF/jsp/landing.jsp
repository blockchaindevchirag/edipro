...
<html lang="en">
    ...
    <body class="auth-prod-slider" style="background: #39465C!important;">
            ...
            <div class="auth-content container">
                <div class="card" style="background: #ffffff !important;">
                    <div class="row align-items-center">
                        <div class="col-xl-12">
                            <div class="card-body">
                                <div class="col-12 text-right logout p-0 m-0"><ul>

                                        <li><a href="logout" class=""><i class="feather icon-power text-danger"></i> Logout</a></li></ul></div>

                                <form id="loginForm" action="reports" autocomplete="off"  method="get" class="d-lg-block d-xl-none login formtop offset-lg-4 offset-sm-0 col-lg-5 offset-md-4 col-sm-10 col-md-6 p-0">
                                    ...
                                    <div class="col-lg-10 col-sm-10 col-md-8 ">
                                        <h6><div class="text-center text-green font-30" style=""><span class="">Welcome!</span></div></h6>

                                        <h4>
                                            <div class="text-center m-t-10 text-green"><span style="font-size:21px;"><%=session.getAttribute("LoginLastname")%></span>, <span style="font-size:21px;"><%=session.getAttribute("LoginFirstname")%></span></div>
                                            <c:set var="practice" scope="session" value='<%=session.getAttribute("LoginPra")%>'/>  
                                            <c:if test="${not empty practice}">
                                                 <div class="text-center m-t-10 text-green"><span style="font-size:15px;font-style: italic;">Practice Name : </span> <span style="font-size:13px; font-weight:bold;"><%=session.getAttribute("LoginPra")%></span></div>
                                            </c:if>
                                            <div class="text-center m-t-10 text-green"><span style="font-size:15px;font-style: italic;">To</span> <span><%=session.getAttribute("EntityName")%></span></div>
                                            
                                        </h4>
                                        <button class="btn btn-gradient-success d-none" id="login" name="login" >Login</button>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalCenterTitle">Welcome <span id="username"></span>,</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    </div>
                    <div class="modal-body">
                        <p>Please, change your password to comply with HIPPA Regulations.</p>
                    </div>
                    <div class="modal-footer">
                        <div class="col-xs-12 col-sm-12" style="display: flex;
                             align-items: center;
                             justify-content: center;">
                            <button type="button" class="btn btn-light"><a href="profilesetting"> Change Password </a></button> 
        ...
        <button type="button" class="btn btn-primary center" id="model1" data-toggle="modal" data-target="#exampleModalCenter"></button>
        ...
        <script >"use strict";
            ...
            $(document).ready(function () {
                ...
                if (typeof (gb) !== "undefined") {
                    if (islogin !== 'Y' && gb === 'N') {
                        localStorage.setItem("passnotify", 'Y');
                ...
                var username = LoginUsername.split('@');
            ...
            var services_data = [{name: "Eligibility Verification", icon: "industries", endpoint: "evlist"},
                ...
                {name: "TX-Estimate", icon: "estimate", endpoint: "preestimate"},
                ...
                {name: "Reports", icon: "reports", endpoint: "reports"},
            ...
            function addService(serv, index) {
                ...
                if (index === 3){
                    ...
                    group.setAttribute("data-placement", "top");
                    // group.setAttribute("data-original-title", "Reduce the clumsy paperwork of patients and the difficulties in storing, retrieving, sharing of these vital documents via electronic form filling, signatures and storage. ");
                    group.setAttribute("data-original-title", "pre-estimate");
                ...
                if (index === 8) {
                    group.setAttribute("data-toggle", "tooltip");
                    group.setAttribute("data-placement", "bottom");
            ...
            function serviceClick(ev) {
                ...
                $('#loginForm').attr('action', services_data[class_index].endpoint);
                $('#loginForm').submit();
                //Hides current text of the main bubble
                jQuery(use_copy).addClass("changing");

                //Sets pointer to the right position
                twn_pointer_path.tweenTo(class_index * (pointer_time / (2 * 8)));
